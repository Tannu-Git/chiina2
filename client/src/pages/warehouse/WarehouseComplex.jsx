...
const Warehouse = () => {
  ...
  const handleQCResult = async (result) => {
    try {
      if (result.success) {
        if (result.summary) {
          const { loopBacksCreated, approvedItems, shortageItems, damagedItems } = result.summary
          if (loopBacksCreated > 0) {
            toast.success(`QC completed! ${approvedItems} items approved, ${loopBacksCreated} loop-back orders created for shortages/damages.`)
          } else {
            toast.success(`QC completed! All ${approvedItems} items approved.`)
          }
        } else {
          toast.success('QC inspection completed successfully!')
        }

        // Refresh warehouse dashboard data
        fetchDashboardData()
      }
    } catch (error) {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8">
                  ...
                  <div key={order._id} className="border border-stone-200 rounded-lg p-4 hover:border-amber-300 hover:shadow-lg hover:bg-amber-50/30 transition-all duration-300 cursor-pointer group">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => handleQCInspection(order)} className="hover:bg-green-50 hover:border-green-300 hover:text-green-700 transition-all duration-200">
                          <CheckCircle className="h-4 w-4 mr-1" />
                          QC
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleCreateLoopback(order)} className="hover:bg-orange-50 hover:border-orange-300 hover:text-orange-700 transition-all duration-200">
                          <RotateCcw className="h-4 w-4 mr-1" />
                          Loop-back
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleAllocateToContainer(order)} className="hover:bg-purple-50 hover:border-purple-300 hover:text-purple-700 transition-all duration-200">
                  ...
                  <div key={container._id} className="border border-stone-200 rounded-lg p-6 hover:border-blue-300 hover:shadow-lg hover:bg-blue-50/30 transition-all duration-300 cursor-pointer group">
                    ...
                    <div className="mt-4 pt-3 border-t flex space-x-2">
                      <Link to={`/containers/${container._id}`} className="flex-1">
                        <Button variant="outline" size="sm" className="w-full hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-all duration-200">
                          <Eye className="h-4 w-4 mr-1" />
                          View Details
                        </Button>
                      </Link>
                      <Link to={`/containers/${container._id}/edit`} className="flex-1">
                        <Button variant="outline" size="sm" className="w-full hover:bg-green-50 hover:border-green-300 hover:text-green-700 transition-all duration-200">
      ...
      <OrderDetailsModal
        ...
        onCreateLoopback={handleCreateLoopback}
        onAllocateContainer={handleAllocateToContainer}
      />
    </div>
  )
}
